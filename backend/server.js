...
const PRIORITY_LABELS = {
  budget: 'עמידה בתקציב',
  speed: 'מהירות תהליך',
  halacha: 'התאמה מדויקת להלכה',
  design: 'איכות ועיצוב',
  support: 'ליווי צמוד לאורך כל הדרך'
};
...
function answerSections(answers) {
  ...
  return [
    ['פרטי קשר', [
      ['שם מלא', val(answers.contact.fullName)],
      ['טלפון', val(answers.contact.phone)],
      ['מייל', val(answers.contact.email)],
      ['וואטסאפ', val(answers.contact.whatsapp)],
      ['עמותה / קהילה / מוסד', val(answers.contact.org)]
    ]],
    ['סוג הפרויקט', [
      ['סוגי פרויקט', types],
      ['אחר - פירוט', val(answers.projectType.typeOther)]
    ]],
    ['גודל ושימוש', [
      ['משתמשים צפויים ביום חול', val(answers.sizeAndUse.weekday)],
      ['בשבתות וחגים', val(answers.sizeAndUse.shabbat)],
      ['חללים נוספים', val(answers.sizeAndUse.extraSpaces)]
    ]],
    ['מיקום וסטטוס', [
      ['יישוב / שכונה', val(answers.locationAndStatus.city)],
      ['האם יש מגרש', val(answers.locationAndStatus.plot)],
      ['סטטוס תכנוני', val(answers.locationAndStatus.planStatus)]
    ]],
    ['לוח זמנים', [
      ['יעד לחנוכת הבית', val(answers.timeline.targetDate)],
      ['לחץ זמנים מיוחד', val(answers.timeline.timePressure)]
    ]],
    ['תקציב ומימון', [
      ['תקציב משוער', val(answers.budgetAndFunding.budget)],
      ['כספים שגויסו', val(answers.budgetAndFunding.raised)],
      ['מקור מימון עיקרי', val(answers.budgetAndFunding.funding)]
    ]],
    ['סדר עדיפויות', Object.entries(PRIORITY_LABELS).map(([key, label]) => [
      label,
      answers.priorities[key] ? `${answers.priorities[key]} / 5` : 'לא דורג'
    ])],
...
async function createDocx({ deal, client, answers }) {
  const children = [
    new Paragraph({
      ...
      children: [new TextRun({ text: 'גוטליב אדריכלים', bold: true, size: 34 })]
    }),
    new Paragraph({
      alignment: AlignmentType.RIGHT,
      bidirectional: true,
      children: [new TextRun({ text: 'שאלון לקוח - סיכום תשובות', bold: true, size: 28 })]
    }),
    new Paragraph({
      alignment: AlignmentType.RIGHT,
      bidirectional: true,
      children: [new TextRun({ text: `עסקה: ${deal.name || deal.id}`, size: 22 })]
    }),
    new Paragraph({
      alignment: AlignmentType.RIGHT,
      bidirectional: true,
      children: [new TextRun({ text: `לקוח: ${client?.name || answers.contact.fullName}`, size: 22 })]
    }),
    new Paragraph({ text: '' })
  ];
...
async function getPdfFontBytes() {
  if (!pdfFontBytesPromise) {
    pdfFontBytesPromise = fs.readFile(
      require.resolve('@expo-google-fonts/noto-sans-hebrew/400Regular/NotoSansHebrew_400Regular.ttf')
    );
  }
...
async function createPdf({ deal, client, answers }) {
  ...
  const NAVY = rgb(0.08, 0.12, 0.17);
  const TEXT = rgb(0.12, 0.15, 0.18);
  const MUTED = rgb(0.42, 0.43, 0.43);
  const TAUPE = rgb(0.70, 0.64, 0.54);
  const PAPER = rgb(0.965, 0.955, 0.935);
  ...
  const textWidth = (text, size) => font.widthOfTextAtSize(text, size);

  const drawRight = (logicalText, {
    size = 11,
    color = TEXT,
  ...
  const wrapLogical = (logicalText, size, maxWidth = CONTENT_W) => {
    const words = String(logicalText ?? '').split(/\s+/).filter(Boolean);
    ...
    for (const word of words) {
      ...
      const visualCandidate = rtlVisual(candidate);
      if (!line || textWidth(visualCandidate, size) <= maxWidth) {
        line = candidate;
      } else {
        lines.push(line);
        line = word;
  ...
  const addPage = (first = false) => {
    ...
    if (first) {
      page.drawRectangle({ x: 0, y: PAGE_H - 128, width: PAGE_W, height: 128, color: NAVY });
      drawRight('גוטליב אדריכלים', {
        size: 12,
        color: TAUPE,
        yPos: PAGE_H - 47
      });
      drawRight('שאלון לקוח - סיכום תשובות', {
        size: 23,
        color: WHITE,
        yPos: PAGE_H - 83
      });
      drawRight('פרטי הפרויקט כפי שנמסרו בשאלון המקדים', {
        size: 10.5,
      ...
      page.drawRectangle({ x: 0, y: PAGE_H - 16, width: PAGE_W, height: 16, color: NAVY });
      y = PAGE_H - 50;
    }
  };

  const ensureSpace = (needed) => {
    if (y - needed < 64) addPage(false);
  };
  ...
  const drawMetaCard = () => {
    ...
    const right = PAGE_W - MARGIN_X - 18;
    drawRight(`עסקה: ${deal.name || deal.id}`, { size: 11, color: TEXT, right, yPos: y - 12 });
    drawRight(`לקוח: ${client?.name || answers.contact.fullName}`, { size: 11, color: TEXT, right, yPos: y - 34 });
    drawRight(`מזהה עסקה: ${deal.id}`, { size: 9.5, color: MUTED, right, yPos: y - 55 });
    y -= cardH + 12;
  };
  ...
  const drawRow = (label, value) => {
    const valueLines = wrapLogical(value, 10.8, CONTENT_W - 18);
    const needed = 18 + valueLines.length * 16 + 10;
    ensureSpace(needed);
  ...
  pages.forEach((pdfPage, index) => {
    ...
    pdfPage.drawText(`GOTLIB ARCHITECTS  |  ${index + 1}/${pages.length}`, {
      x: MARGIN_X,
      y: 25,
      size: 8,
      font: latinFont,
...
async function createMondayUpdate(dealId) {
  const body = `שאלון לקוח התקבל בתאריך ${new Intl.DateTimeFormat('he-IL', {
    timeZone: 'Asia/Jerusalem',
    dateStyle: 'short',
    timeStyle: 'short'
  }).format(new Date())}. מצורפים קובצי Word ו-PDF.`;
...
async function uploadFileToUpdate(updateId, buffer, filename, mimeType) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok || data.errors || !data.data?.add_file_to_update?.id) {
    throw new Error(`Monday file upload failed: ${JSON.stringify(data.errors || data)}`);
  }
...
app.get('/send-questionnaire', async (req, res) => {
  ...
  try {
    ...
    if (!deal) return res.status(404).send(page('העסקה לא נמצאה', 'לא נמצאה עסקה מתאימה בלוח העסקאות.', false));

    const client = await getLinkedClient(deal);
    if (!client) return res.status(400).send(page('לקוח לא נמצא', 'לא נמצא לקוח תקין שמקושר לעסקה הזאת ב-Monday.', false));

    const email = String(client.column_values?.[0]?.text || '').trim();
    if (!email || !email.includes('@')) return res.status(400).send(page('חסר אימייל', 'ללקוח המקושר אין כתובת אימייל תקינה.', false));
...
app.post('/api/questionnaire/submit', async (req, res) => {
  ...
  try {
    const deal = await getDeal(dealId);
    if (!deal) return res.status(404).json({ ok: false, error: 'Deal not found on the configured deals board' });

    const client = await getLinkedClient(deal);
    const [docxBuffer, pdfBuffer] = await Promise.all([
      createDocx({ deal, client, answers }),
      createPdf({ deal, client, answers })
    ]);

    const clientName = safeFilePart(client?.name || answers.contact.fullName, 'לקוח');
    const dealName = safeFilePart(deal.name, `עסקה ${dealId}`);
    const baseName = `שאלון לקוח - ${clientName} - ${dealName}`;

    const updateId = await createMondayUpdate(dealId);
    const docxAsset = await uploadFileToUpdate(
      updateId,
      docxBuffer,
      `${baseName}.docx`,
      'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    );
    const pdfAsset = await uploadFileToUpdate(
      updateId,
      pdfBuffer,
      `${baseName}.pdf`,
      'application/pdf'
    ...
    console.log(`Questionnaire completed for deal ${dealId}`, {
      updateId,
    ...
    return res.status(502).json({ ok: false, error: 'Could not generate or upload questionnaire documents' });
  }
});
